...
public class FindMaximum {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < string.length(); i++) { // Vòng lặp bên ngoài
            ...
            // So sánh kích cỡ của list trung gian và list chứa chuỗi tăng dần có kích thước
            // lớn nhất
            if (list.size() > max.size()) {
                // Nếu kích cỡ của list trung gian lớn hơn kích cỡ của list có độ dài lớn nhất
                // thì gán lại max
                max.clear();
                max.addAll(list);
            }
